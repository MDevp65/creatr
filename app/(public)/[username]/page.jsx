...
const UserNamePage = ({ params }) => {
    ...
    } = useConvexQuery(api.public.getPublishedPostsByUsername, { username, limit: 20 })

    // Get follower count
    const {
        data: followerCount
    } = useConvexQuery(api.followers.getFollowersCount, user ? { userId: user?._id } : "skip")

    // Check if current user already following this profile
    const { data: isFollowing } = useConvexQuery(
        api.followers.isFollowing, currentUser && user ? { followingId: user?._id } : "skip"
    )

    const toggleFollow = useConvexMutation(api.followers.toggleFollow)
        ...
        <div className='min-h-screen bg-slate-900 text-white'>
            ...
            <div className="max-w-7xl mx-auto px-6 py-12">
                ...
                <div className="text-center mb-12">
                    <div className="relative w-24 h-24 mx-auto mb-6 mt-6 flex items-center justify-center">
                        {user?.imageUrl ? (
