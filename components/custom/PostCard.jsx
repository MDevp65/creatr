...
const PostCard = ({
    post, showActions = false, showAuthor = true, onEdit, onDelete, className = ""
}) => {
    ...
    return (
        <Card className={`card-glass hover:border-purple-500/50 transition-colors ${className}`}>
            <CardContent>
                <div className="space-y-4">
                    <Link href={publicUrl || "#"}
                        className={!publicUrl ? "pointer-events-none" : ""}
                        target='_blank'>
                        <div className="relative w-full h-48 rounded-lg onverflow-hidden">
                            <Image
                    ...
                    <div className="flex items-start justify-between mt-5">
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                {post.scheduleFor && post.scheduleFor > Date.now() && (
                                    <div className="flex items-center text-xs text-blue-400">
                                        <Calendar className='h-3 w-3 mr-1' />
                                        {new Date(post.scheduleFor).toLocaleDateString()}
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {post.tags.slice(0, 3).map((tag) => (
                                <Badge
                                    key={tag}
                                    variant={"secondary"}
                                    className={"bg-purple-500/20 text-purple-300 border-purple-500/20 text-xs"}
                                >
                                    {tag}
                                </Badge>
                            ))}
